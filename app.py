...
def render_liquidity(panel: dict):
    ...
    if oi_data and not oi_data.get("error"):
        # Show current OI only (what works)
        data_source = oi_data.get("data_source", "Unknown")
        current_oi = oi_data.get("current_oi", 0)
        
        st.markdown(f"""
        <div style="background:#f0f2f6;padding:16px;border-radius:8px;margin-bottom:16px;">
            <div style="font-size:0.8rem;color:#888;margin-bottom:8px;">Data: {data_source}</div>
            <div style="font-size:1.1rem;font-weight:700;color:#333;margin-bottom:12px;">
                Current OI: {current_oi:,.0f} BTC
            </div>
            <div style="font-size:0.85rem;color:#666;line-height:1.6;">
                <strong>📊 For OI Trends & Squeeze Detection:</strong><br>
                Visit <a href="https://www.coinglass.com/BitcoinOpenInterest" target="_blank" style="color:#1db954;font-weight:600;">Coinglass.com</a> 
                to check:<br>
                • OI 24h Change (Rising/Falling)<br>
                • Funding Rate (Positive/Negative)<br>
                • Price 24h Change<br><br>
                <strong>Combine with scanner data below:</strong><br>
                • Check Liquidity Sweep (BULL/BEAR)<br>
                • Check 1H Impulse direction<br>
                • Match patterns to detect squeeze setups
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        # Complete failure - show manual instructions
        st.info("ℹ️ **Open Interest Auto-Detection Unavailable** (Network restrictions)")
        st.markdown("""
...
def main():
    ...
    default_watchlist = "BTC, ETH, SOL, DOGE, PEPE, WIF, AVAX, LINK, MATIC, AAVE"  # 10 pairs (ultra-fast)  # 15 pairs (Streamlit Cloud optimized)
